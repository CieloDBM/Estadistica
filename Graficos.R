...
# 1.- Creación de un vector de color
color=c("coral", "cadetblue2", "darkolivegreen")

# 2.- Creación del gráfico
GB1<-ggplot(BD1, aes(x=año))+
  geom_bar(colour= "bisque", fill=color)+
...
HG<-ggplot(BD1, aes(x=largo_aleta_mm))+
  geom_histogram(col="chocolate1", fill="darkseagreen3")+
  ggtitle("Histograma")+
...
GD<-ggplot(BD1, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  theme_light()
#es bd1 porque es nuestra matriz modificada, es decir, eliminamos la primera columna de penguins
# 2.- Visualizacion del objeto
#hay que activas estas letras que indican que grafico
GD
...
GD3<-ggplot(BD1, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c("gold","pink","aquamarine"))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()
